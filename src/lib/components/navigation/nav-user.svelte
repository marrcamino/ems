<script lang="ts">
  ...
  // The name and position live on the employee record now, not the login.
  const name = $derived(fullName(user.employee));
   ...
   */
  const subtitle = $derived(user.employee.positionTitle || user.username);
  ...
  <Sidebar.MenuItem>
    <DropdownMenu.Root>
      ...
      >
        ...
        {#if user.employee.positionTitle}
